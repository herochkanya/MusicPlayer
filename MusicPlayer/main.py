        ...
        def hook(d):
            if d['status'] == 'downloading':
                ...
                if total > 0:
                    ...
                    msg = f"⬇️ {filename} {percent}% — {downloaded//1_000_000}/{total//1_000_000} MB — {int(speed/1024)} KB/s"
                    self.backend.log_signal.emit(msg)
                else:
                    msg = f"⬇️ Downloading: {downloaded//1_000_000} MB..."
                    self.backend.log_signal.emit(msg)
...
class Backend(QObject):
    ...
    tray_mode_changed = Signal(bool) # Tray mode change signal
...
class MainWebView(QWebEngineView):
    ...
    def closeEvent(self, event):
        tray_enabled = self.backend.get_tray_mode()

        if tray_enabled and not self.is_really_quitting:
            event.ignore()
            self.hide()
...
def main():
    ...
    tray_mode_action = QAction("Minimize to Tray", tray_menu, checkable=True)
    tray_mode_action.setChecked(get_tray_mode())

    def on_tray_toggle(state):
        backend.set_tray_mode(state)
    tray_mode_action.triggered.connect(on_tray_toggle)

    backend.tray_mode_changed.connect(tray_mode_action.setChecked)
    ...
    tray_menu.addAction(tray_mode_action)
